...
class Frame extends _instrumentation.SdkObject {
  ...
  _context(world, useContextPromise = false) {
    if (process.env['TURNSTILEBROWSER_PATCHES_RUNTIME_FIX_MODE'] === '0' || this._contextData.get(world).context || useContextPromise) {
    ...
    return crSession.__re__emitExecutionContext({
      ...
      console.error('[turnstilebrowser-patches][frames._context] cannot get world, error:', error);
    });
  }
  _mainContext() {
  ...
  async evaluateExpression(expression, options = {}, arg) {
    var _options$world;
    const context = await this._context((_options$world = options.world) !== null && _options$world !== void 0 ? _options$world : 'utility');
    const value = await context.evaluateExpression(expression, options, arg);
    return value;
  }
  async evaluateExpressionHandle(expression, options = {}, arg) {
    var _options$world2;
    const context = await this._context((_options$world2 = options.world) !== null && _options$world2 !== void 0 ? _options$world2 : 'utility');
