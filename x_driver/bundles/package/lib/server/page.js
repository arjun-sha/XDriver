...
class Worker extends _instrumentation.SdkObject {
  ...
  async getExecutionContext() {
    if (process.env['TURNSTILEBROWSER_PATCHES_RUNTIME_FIX_MODE'] !== '0' && !this._existingExecutionContext) {
...
class InitScript {
  constructor(source, internal) {
    ...
    const minLength = 5; // Minimum length of the random string
    const maxLength = 15;
    ...
    this.source = `(() => {
      ...
      const hasInitScript = globalThis.${result}[${JSON.stringify(guid)}];
      if (hasInitScript)
        return;
      globalThis.${result}[${JSON.stringify(guid)}] = true;
      ${source}
    })();`;
