...
class CRSession extends _events.EventEmitter {
  ...
  async __re__emitExecutionContext({
    ...
    const utilityWorldName = process.env['TURNSTILEBROWSER_PATCHES_UTILITY_WORLD_NAME'] !== '0' ? process.env['TURNSTILEBROWSER_PATCHES_UTILITY_WORLD_NAME'] || 'util' : '__playwright_utility_world__';
    process.env['TURNSTILEBROWSER_PATCHES_DEBUG'] && console.log(`[turnstilebrowser-patches][crSession] targetId = ${targetId}, world = ${world}, frame = ${frame ? 'Y' : 'N'}, fixMode = ${fixMode}`);
    let getWorldPromise;
  ...
  async __re__getMainWorld({
    ...
    process.env['TURNSTILEBROWSER_PATCHES_DEBUG'] && console.log(`[turnstilebrowser-patches][getMainWorld] binding name = ${randomName}`);
    ...
    const bindingCalledHandler = ({
      ...
      process.env['TURNSTILEBROWSER_PATCHES_DEBUG'] && console.log('[turnstilebrowser-patches][bindingCalledHandler]', {
        name,
        payload,
        executionContextId
      });
    ...
    process.env['TURNSTILEBROWSER_PATCHES_DEBUG'] && console.log(`[turnstilebrowser-patches][getMainWorld] result:`, {
      contextId
    });
    return contextId;
  }
  async __re__getIsolatedWorld({
    ...
    process.env['TURNSTILEBROWSER_PATCHES_DEBUG'] && console.log(`[turnstilebrowser-patches][getIsolatedWorld] result:`, createIsolatedWorldResult);
